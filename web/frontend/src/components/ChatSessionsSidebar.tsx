import { useEffect, useMemo, useState } from 'react'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { Plus, Trash2, Edit2, MessageSquare, Search, Sparkles } from 'lucide-react'
import { Button } from './ui/button'
import { Input } from './ui/input'
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from './ui/dialog'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from './ui/select'
import { Label } from './ui/label'
import { useChatEvents } from '../contexts/ChatEventsContext'
import { useToast } from './ui/toast'
import { ApiError, request } from '@/lib/api'
import {
  ChatSessionSummary,
  chatModelsQueryKey,
  chatSessionsQueryKey,
  fetchChatModels,
  fetchChatSessions,
} from '@/hooks/chatQueries'

interface ChatSessionsSidebarProps {
  transcriptionId: string
  activeSessionId?: string
  onSessionChange: (id: string | null) => void
}

export function ChatSessionsSidebar({ transcriptionId, activeSessionId, onSessionChange }: ChatSessionsSidebarProps) {
  const { subscribeSessionTitleUpdated, subscribeTitleGenerating } = useChatEvents()
  const { toast } = useToast()
  const queryClient = useQueryClient()
  const [selectedModel, setSelectedModel] = useState<string>('')
  const [showNewSessionDialog, setShowNewSessionDialog] = useState(false)
  const [newSessionTitle, setNewSessionTitle] = useState('')
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editTitle, setEditTitle] = useState('')
  const [generatingTitleIds, setGeneratingTitleIds] = useState<Set<string>>(new Set())

  const { data: models = [], isPending: modelsPending } = useQuery({
    queryKey: chatModelsQueryKey,
    queryFn: fetchChatModels,
  })

  const {
    data: sessionsData,
    isPending: sessionsPending,
  } = useQuery({
    queryKey: chatSessionsQueryKey(transcriptionId),
    queryFn: () => fetchChatSessions(transcriptionId),
    enabled: !!transcriptionId,
  })

  const sessions = useMemo(() => sessionsData ?? [], [sessionsData])

  useEffect(() => {
    if (!selectedModel && models.length > 0) {
      setSelectedModel(models[0])
    }
  }, [models, selectedModel])

  useEffect(() => {
    const unsubscribe = subscribeSessionTitleUpdated(({ sessionId, title }) => {
      queryClient.setQueryData(chatSessionsQueryKey(transcriptionId), (prev?: ChatSessionSummary[]) =>
        prev ? prev.map(s => (s.id === sessionId ? { ...s, title } : s)) : prev,
      )
    })
    return unsubscribe
  }, [queryClient, transcriptionId, subscribeSessionTitleUpdated])

  useEffect(() => {
    const unsubscribe = subscribeTitleGenerating(({ sessionId, isGenerating }) => {
      setGeneratingTitleIds(prev => {
        const next = new Set(prev)
        if (isGenerating) {
          next.add(sessionId)
        } else {
          next.delete(sessionId)
        }
        return next
      })
    })
    return unsubscribe
  }, [subscribeTitleGenerating])

  const createSessionMutation = useMutation({
    mutationFn: (payload: { transcription_id: string; model: string; title?: string }) =>
      request<ChatSessionSummary>('/api/v1/chat/sessions', {
        method: 'POST',
        json: payload,
      }),
    onSuccess: created => {
      queryClient.setQueryData(chatSessionsQueryKey(transcriptionId), (prev?: ChatSessionSummary[]) =>
        prev ? [created, ...prev] : [created],
      )
      onSessionChange(created.id)
      setShowNewSessionDialog(false)
      setNewSessionTitle('')
      toast({ title: 'Chat session created' })
    },
    onError: (err: unknown) => {
      const description = err instanceof ApiError ? err.message : 'Failed to create chat session.'
      toast({ title: 'Unable to create', description })
    },
  })

  const updateSessionTitleMutation = useMutation({
    mutationFn: ({ id, title }: { id: string; title: string }) =>
      request<ChatSessionSummary>(`/api/v1/chat/sessions/${id}/title`, {
        method: 'PUT',
        json: { title },
      }),
    onSuccess: updated => {
      queryClient.setQueryData(chatSessionsQueryKey(transcriptionId), (prev?: ChatSessionSummary[]) =>
        prev ? prev.map(s => (s.id === updated.id ? updated : s)) : prev,
      )
      setEditingId(null)
    },
    onError: (err: unknown) => {
      const description = err instanceof ApiError ? err.message : 'Failed to update title.'
      toast({ title: 'Update failed', description })
    },
  })

  const deleteSessionMutation = useMutation({
    mutationFn: (id: string) =>
      request(`/api/v1/chat/sessions/${id}`, {
        method: 'DELETE',
      }),
    onSuccess: (_, id) => {
      queryClient.setQueryData(chatSessionsQueryKey(transcriptionId), (prev?: ChatSessionSummary[]) =>
        prev ? prev.filter(s => s.id !== id) : prev,
      )
      if (activeSessionId === id) {
        const remaining = queryClient.getQueryData<ChatSessionSummary[]>(chatSessionsQueryKey(transcriptionId)) ?? []
        onSessionChange(remaining[0]?.id ?? null)
      }
      toast({ title: 'Chat session deleted' })
    },
    onError: (err: unknown) => {
      const description = err instanceof ApiError ? err.message : 'Failed to delete session.'
      toast({ title: 'Deletion failed', description })
    },
  })

  const creating = createSessionMutation.isPending
  const deletingId = deleteSessionMutation.variables
  const loading = sessionsPending || modelsPending

  const handleCreateSession = () => {
    if (!selectedModel || creating) return
    createSessionMutation.mutate({
      transcription_id: transcriptionId,
      model: selectedModel,
      title: newSessionTitle || undefined,
    })
  }

  const handleUpdateTitle = (id: string, title: string) => {
    if (title.trim() === '') {
      setEditingId(null)
      return
    }
    updateSessionTitleMutation.mutate({ id, title })
  }

  const handleDeleteSession = (id: string) => {
    if (!window.confirm('Delete this chat session?')) return
    deleteSessionMutation.mutate(id)
  }

  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-850">
      <div className="flex-shrink-0 p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Chats</h2>
          <Dialog open={showNewSessionDialog} onOpenChange={setShowNewSessionDialog}>
            <DialogTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="h-8 w-8 p-0 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-200 dark:hover:bg-gray-800"
                title="New Chat"
              >
                <Plus className="h-4 w-4" />
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px] bg-white dark:bg-gray-800 shadow-2xl">
              <DialogHeader>
                <DialogTitle>New Chat Session</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="model">Model</Label>
                  <Select value={selectedModel} onValueChange={setSelectedModel} disabled={models.length === 0}>
                    <SelectTrigger className="w-full bg-white dark:bg-gray-800 border-0 text-foreground">
                      <SelectValue placeholder={modelsPending ? 'Loading models...' : 'Select a model'} />
                    </SelectTrigger>
                    <SelectContent className="bg-white dark:bg-gray-850 border-0">
                      {(models || []).map(m => (
                        <SelectItem key={m} value={m}>
                          {m}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="title">Title (optional)</Label>
                  <Input id="title" value={newSessionTitle} onChange={e => setNewSessionTitle(e.target.value)} placeholder="Enter a title..." />
                </div>
                <Button onClick={handleCreateSession} className="w-full" disabled={!selectedModel || creating}>
                  {creating ? 'Creating...' : 'Create Session'}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>

        <div className="relative mb-4">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
          <Input placeholder="Search conversations..." className="pl-10 bg-white dark:bg-gray-800 border-0 text-sm" disabled />
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-2 pb-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            <MessageSquare className="h-12 w-12 mb-4 text-gray-300 dark:text-gray-600 animate-pulse" />
            <div className="text-sm text-center">
              <p className="font-medium">Loading conversations...</p>
            </div>
          </div>
        ) : sessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            <MessageSquare className="h-12 w-12 mb-4 text-gray-300 dark:text-gray-600" />
            <div className="text-sm text-center">
              <p className="font-medium">No conversations yet</p>
              <p className="mt-1 text-xs">Start a new chat to begin!</p>
            </div>
          </div>
        ) : (
          <div className="space-y-1">
            {sessions.map(s => (
              <div
                key={s.id}
                className={`group relative flex items-center p-2 mx-2 rounded-lg cursor-pointer transition-all duration-150 ${
                  activeSessionId === s.id ? 'bg-gray-200 dark:bg-gray-800' : 'hover:bg-gray-100 dark:hover:bg-gray-800'
                }`}
                onClick={() => onSessionChange(s.id)}
              >
                <div className="flex-shrink-0 mr-3">
                  <MessageSquare className="h-4 w-4 text-gray-400 dark:text-gray-500" />
                </div>

                <div className="flex-1 min-w-0 flex items-center">
                  {editingId === s.id ? (
                    <Input
                      value={editTitle}
                      onChange={e => setEditTitle(e.target.value)}
                      onKeyDown={e => {
                        if (e.key === 'Enter') handleUpdateTitle(s.id, editTitle)
                        if (e.key === 'Escape') setEditingId(null)
                      }}
                      onBlur={() => handleUpdateTitle(s.id, editTitle)}
                      className="h-6 text-sm bg-white dark:bg-gray-900 border-0 p-0 focus-visible:ring-0"
                      autoFocus
                    />
                  ) : (
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      <div className="truncate text-sm text-gray-900 dark:text-gray-100 font-medium">{s.title || 'New Chat'}</div>
                      {generatingTitleIds.has(s.id) && (
                        <div className="flex-shrink-0 text-blue-500 dark:text-blue-400" title="Generating title...">
                          <Sparkles className="h-3.5 w-3.5 animate-pulse" />
                        </div>
                      )}
                    </div>
                  )}
                </div>

                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150" onClick={event => event.stopPropagation()}>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-7 w-7 text-gray-400 hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-100"
                    onClick={() => {
                      setEditingId(s.id)
                      setEditTitle(s.title ?? '')
                    }}
                    title="Rename session"
                  >
                    <Edit2 className="h-3.5 w-3.5" />
                  </Button>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-7 w-7 text-red-400 hover:text-red-600 dark:text-red-500 dark:hover:text-red-400"
                    onClick={() => handleDeleteSession(s.id)}
                    title="Delete session"
                    disabled={deleteSessionMutation.isPending && deletingId === s.id}
                  >
                    <Trash2 className="h-3.5 w-3.5" />
                  </Button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}
